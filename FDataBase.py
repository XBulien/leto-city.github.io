...
class FDataBase:
    ...
    def taxCertify(self, id, tax, isFirm):
        user = self.getUser(id)
        if user:
            self.__cur.execute(f'UPDATE users SET PaidTaxes=2 WHERE id={id}')
            self.__db.commit()
        else:
            return False
        if isFirm:
            return float(tax) * 0.2
        return float(tax) * 0.13
    ...
    def updateUser(self, id):
        try:
            user = self.getUser(id)
            if user['PaidTaxes'] == 0 or user["PaidTaxes"] == 2:
                self.__cur.execute(f'UPDATE users SET PaidTaxes=1 WHERE id={id}')
            else:
                self.__cur.execute(f'UPDATE users SET PaidTaxes=0 WHERE id={id}')
            self.__db.commit()
        except sqlite3.Error as e:
            print("Ошибка добавления пользователя в БД", e)
            return False
