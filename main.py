...
DEBUG = True
DATABASE = 'C:\\Users\\Player\\Documents\\Web_Site_for_leto-city\\data.db'
...
def register():
    if request.method == "POST":
        ...
        if len(request.form['psw']) > 4 and request.form['psw'] == request.form['psw2'] and dbase.getUser(request.form['id'])['psw'] == ".":
            id = request.form['id']
...
@app.route('/admin/business_taxes', methods = ["POST", "GET"])
@login_required
def business_taxes():
     user = dbase.getUser(current_user.id)
     if user['IsAdmin'] != 1:
        ...
        if request.method == "POST":
            ...
            isFirm = True
            hasToPay = dbase.taxCertify(id, tax, isFirm)
            if hasToPay is False:
                flash('Пользователь не найден!','error')
            else:
                flash(f'Квитанция успешно отправлена в банк. Сумма выплат - {hasToPay}.', "success")
        return render_template('business_taxes.html')
@app.route('/admin/civil_taxes', methods = ["POST", "GET"])
@login_required
def civil_taxes():
     user = dbase.getUser(current_user.id)
     if user['IsAdmin'] != 1:
        ...
        if request.method == "POST":
            ...
            if hasToPay is False:
                ...
                flash(f'Квитанция успешно отправлена в банк. Сумма выплат - {hasToPay}.', "success")
        return render_template('civil_taxes.html')
@app.route('/admin/stats', methods = ["POST", "GET"])
@login_required
def admin_stats():
    user = dbase.getUser(current_user.id)
    if user['IsAdmin'] != 1:
        ...
        if request.method == "POST":
            ...
            print(id)
            res = dbase.updateUser(int(id))
            print(res)
            if res:
                flash("Успешно обновлено", "success")
            else:
                flash("Ошибка при изменении в БД", "error")
        return render_template('stats.html', citizens = dbase.getUsers())
@app.route('/profile')
@login_required
def profile():
...
def admin_messages():
    ...
    if user['IsAdmin'] != 1:
        ...
        return render_template('messages.html', dbase = dbase, msgs = dbase.getMsgs())
